...
namespace com.rfilkov.components
{
    ...
    public class CubePresentationScript : MonoBehaviour
    {
        ...
        [Tooltip("UI-Text to display scene information.")]
        public UnityEngine.UI.Text sceneInfo;
        ...
        void Start()
        {
            ...
            if (side < cubeSides.Count && cubeSides[side] && cubeSides[side].GetComponent<Renderer>())
            {
                ...
                //cubeSides[side].GetComponent<Renderer>().material.SetTexture("_BaseMap", slideTextures[tex]);  // URP
            }
        ...
        private void SceneInfo()
        {
            if (side == 0)
            {
                sceneInfo.text = "Go to the snowy mountain with a T-pose£¡";
            }
            if (side == 1)
            {
                sceneInfo.text = "Go to the park with a T-pose£¡";
            }
            if (side == 2)
            {
                sceneInfo.text = "Go to the desert with a T-pose£¡";
            }
            if (side == 3)
            {
                sceneInfo.text = "Go to the rainforest with a T-pose£¡";
            }
        }
    }
}
