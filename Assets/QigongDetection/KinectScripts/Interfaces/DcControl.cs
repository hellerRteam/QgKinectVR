...
namespace com.rfilkov.components
{
    ...
    public class DcControl : MonoBehaviour
    {
        ...
        protected void DoMoveAvatar(ulong UserID, Vector3 trans)
        {
            ...
            if (posRelativeToCamera)
            {
                ...
                if (posRelOverlayColor || !offsetCalibrated)
                {
                    ...
                    offsetCalibrated = false;
                    //Debug.Log("bodyRootPosition: " + bodyRootPosition);
                }
            }
            ...
            if (groundedFeet && verticalMovement)  // without vert-movement, there happens an ever expanding jump up & down
            {
                ...
                if (Mathf.Abs(fNewDistance) >= 0.01f && Mathf.Abs(fNewDistance - fFootDistanceInitial) >= maxFootDistanceGround)
                {
                    if ((fNewDistanceTime - fFootDistanceTime) >= maxFootDistanceTime)
                    {
                        ...
                        //Debug.Log("footDist: " + fFootDistance + ", newDist: " + fNewDistance + ", initDist: " + fFootDistanceInitial);

                        //targetPos.y = fLastTgtY + fFootDistance;
                        vFootCorrection = initialUpVector * fFootDistance;
                        targetPos = lastTargetPos + vFootCorrection;
        ...
        protected virtual float GetCorrDistanceToGround(Transform trans, Vector3 initialPos, bool isRightJoint)
        {
            ...
            //Debug.Log("right: " + isRightJoint + ", dist: " + deltaDist + ", vTrans: " + vTrans + ", deltaDir: " + deltaDir + ", transPos: " + trans.position + ", initPos: " + initialPos + ", upVector: " + initialUpVector);
