...
namespace com.rfilkov.components
{
    ...
    public class InteractionManager : MonoBehaviour
    {
        ...
        [SerializeField]public float certainTime;

        public Sprite spritef1;
        public Sprite spritef2;
        public Sprite spritem1;
        public Sprite spritem2;
        public Sprite yCon;
        public Sprite nCon;
        public float[] timer = new float[2];
        ...
        /// </summary>
        /// <param name="playerIndex">Player index, tracked by the IM.</param>
        /// <param name="leftHandInteraction">Whether the IM tracks left-hand interactions or not.</param>
        /// <param name="rightHandInteraction">Whether the IM tracks right-hand interactions or not.</param>
        /// <returns></returns>
        ...
        private void UpdateGUI()
        {
            ...
            if (guiHandCursor)
            {
                ...
                if ((cursorTexture != null) /**&& (isLeftHandPrimary || isRightHandPrimary)*/)
                {
                    ...
                    //if (RectTransformUtility.RectangleContainsScreenPoint(fmImage.rectTransform, guiHandCursor.rectTransform.anchoredPosition, camera))
                    if(guiHandCursor.rectTransform.anchoredPosition.x >= 370.5 && guiHandCursor.rectTransform.anchoredPosition.y>= 112.3 &&
                    guiHandCursor.rectTransform.anchoredPosition.x<=540 && guiHandCursor.rectTransform.anchoredPosition.y<=204.1)
                    {
                        timer[0] += Time.deltaTime;
                        if (timer[0] >= certainTime) 
                        {
                            isF = true;
                            isM = false;
                            fmImage.sprite = spritef2;
                            mImage.sprite = spritem1;
                        }
                    ...
                    //else if (RectTransformUtility.RectangleContainsScreenPoint(mImage.rectTransform, guiHandCursor.rectTransform.anchoredPosition, camera)) 
                    else if (guiHandCursor.rectTransform.anchoredPosition.x >= 76 && guiHandCursor.rectTransform.anchoredPosition.y >= 115 &&
                    guiHandCursor.rectTransform.anchoredPosition.x <= 243.5 && guiHandCursor.rectTransform.anchoredPosition.y <= 204.1)
                    {
                        timer[0] += Time.deltaTime;
                        if (timer[0] >= certainTime)
                        {
                            isM = true;
                            isF = false;
                            fmImage.sprite = spritef1;
                            mImage.sprite = spritem2;
                        }
                            /*if (rightHandState == KinectInterop.HandState.Closed)
                            {
                                isM = true;
                    ...
                    */
                    ...
                    {
                        cImage.sprite = yCon;
                        //if (RectTransformUtility.RectangleContainsScreenPoint(cImage.rectTransform, guiHandCursor.rectTransform.anchoredPosition, camera))
                        if (guiHandCursor.rectTransform.anchoredPosition.x >= 226.9 && guiHandCursor.rectTransform.anchoredPosition.y >= 34.6 &&
                        guiHandCursor.rectTransform.anchoredPosition.x <= 413.7 && guiHandCursor.rectTransform.anchoredPosition.y <= 70)
                        {
                            timer[1] += Time.deltaTime;
                            if (isF && timer[1] >= certainTime)
                            {
                                SceneManager.LoadScene("QgSceneTrain");
                            }
                            else if (isM && timer[1] >= certainTime)
                            {
                                SceneManager.LoadScene("QgSceneTrain");
                            }
                        }
                    ...
                    float f = timer.Max();
                    cursorProgressBar.fillAmount = f / certainTime;
