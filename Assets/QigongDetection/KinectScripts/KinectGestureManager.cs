...
namespace com.rfilkov.kinect
{
    ...
    public enum GestureType
    {
        ...
        Qg_PrePose = 101,//掌抱腹前-预备式
        QgMove1_1 = 102,//两手托天理三焦
        QgMove2_1 = 103,//左右开弓似射雕搭腕
        QgMove2_2L = 104,//左右开弓似射雕，左开弓
        QgMove2_2R = 105,//左右开弓似射雕，右开弓
        QgMove3_2L = 106,//调理脾胃需单举，举左手
        QgMove3_2R = 107,//调理脾胃需单举，举右手
        QgMove4_2 = 108,//五劳七伤往后瞧
        QgMove5_2 = 109,//摇头摆尾去心火，左倾/左旋
        QgMove5_3 = 110,//摇头摆尾去心火，右倾/右旋
        QgMove5_4 = 111,//摇头摆尾去心火，左旋或右旋时身体向前倾
        QgMove6_1 = 112,//两手攀足固肾腰
        QgMove7_1 = 113,//攒拳怒目增气力，抱拳
        QgMove7_2L = 114,//攒拳怒目增气力，出左拳
        QgMove7_2R = 115,//攒拳怒目增气力，出右拳
        QgMove8_1 = 116,//背后七颠百病消，踮脚
        MoreQgMove = 117,
    ...
    public class KinectGestureManager : MonoBehaviour
    {
        ...
        public struct GestureData
        {
            ...
            public float initialFootR;
            public float initialFootL;
        }
        ...
        [Tooltip("Prefab used to display the joint positions on screen.")]
        public GameObject jointDebugPrefab;
        ...
        protected int leftFootIndex;
        protected int rightFootIndex;

        protected int leftEarIndex;
        protected int rightEarIndex;
        protected int noseIndex;
        ...
        // game objects used for joint debugging
        private GameObject[] debugJoints = null;
        ...
        public void DetectGesture(ulong UserId, GestureType gesture)
        {
            ...
            //        gestureData.checkForGestures.Add(GestureType.Qg_PrePose);
            //        gestureData.checkForGestures.Add(GestureType.QgMove1_1);
            //        gestureData.checkForGestures.Add(GestureType.QgMove2_2L);
            //        gestureData.checkForGestures.Add(GestureType.QgMove2_2R);
            //        gestureData.checkForGestures.Add(GestureType.QgMove3_2L);
            //        gestureData.checkForGestures.Add(GestureType.QgMove3_2R);
            //        gestureData.checkForGestures.Add(GestureType.QgMove4_2);
            //        gestureData.checkForGestures.Add(GestureType.QgMove5_2);
            //        gestureData.checkForGestures.Add(GestureType.QgMove5_3);
            //        gestureData.checkForGestures.Add(GestureType.QgMove5_4);
            //        gestureData.checkForGestures.Add(GestureType.QgMove6_1);
            //        gestureData.checkForGestures.Add(GestureType.QgMove7_1);
            //        gestureData.checkForGestures.Add(GestureType.QgMove7_2L);
            //        gestureData.checkForGestures.Add(GestureType.QgMove7_2R);
            //        gestureData.checkForGestures.Add(GestureType.QgMove8_1);
            //        break;
        ...
        protected void CheckForGestures(ulong UserId, KinectManager kinectManager)
        {
            ...
            if (Time.realtimeSinceStartup >= gesturesTrackingAtTime[UserId])
            {
                ...
                if (jointDebugPrefab != null && debugJoints == null)
                {
                    // array holding the skeleton joints
                    debugJoints = new GameObject[iNeededJointsCount];

                    for (int i = 0; i < iNeededJointsCount; i++)
                    {
                        debugJoints[i] = Instantiate(jointDebugPrefab) as GameObject;
                        debugJoints[i].transform.parent = transform;
                        debugJoints[i].name = ((KinectInterop.JointType)aiNeededJointIndexes[i]).ToString();
                        debugJoints[i].SetActive(false);
                    }
                }
                ...
                for (int i = 0; i < iNeededJointsCount; i++)
                {
                    ...
                    if (joint >= 0)
                    {
                        ...
                        if(debugJoints != null)
                        {
                            debugJoints[i].transform.position = playerJointsPos[joint];
                            debugJoints[i].SetActive(playerJointsTracked[joint]);
                        }
                    }
                }
        ...
        public virtual int[] GetNeededJointIndexes()
        {
            ...
            leftEarIndex = (int)KinectInterop.JointType.EarLeft;
            rightEarIndex = (int)KinectInterop.JointType.EarRight;
            noseIndex = (int)KinectInterop.JointType.Nose;

            leftFootIndex = (int)KinectInterop.JointType.FootLeft;
            rightFootIndex = (int)KinectInterop.JointType.FootRight;
            ...
            int[] neededJointIndexes = {
                ...
                leftEarIndex, rightEarIndex, noseIndex, leftFootIndex, rightFootIndex
            };
        ...
        public virtual void CheckForGesture(ulong userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked)
        {
            ...
            float bandTopY = jointsPos[rightShoulderIndex].y > jointsPos[leftShoulderIndex].y ? jointsPos[rightShoulderIndex].y : jointsPos[leftShoulderIndex].y;//最高的肩
            float bandBotY = jointsPos[rightHipIndex].y < jointsPos[leftHipIndex].y ? jointsPos[rightHipIndex].y : jointsPos[leftHipIndex].y;//最低的臀部

            float bandCenter = (bandTopY + bandBotY) / 2f;//躯干正中间位置
            float bandSize = (bandTopY - bandBotY);//躯干长度

            float gestureTop = bandCenter + bandSize * 1.2f / 2f;//圈定识别姿势的上限
            float gestureBottom = bandCenter - bandSize * 1.3f / 4f;//下限
            float gestureRight = jointsPos[rightHipIndex].x;
            float gestureLeft = jointsPos[leftHipIndex].x;

            Vector3 lvKneeToHip = jointsPos[leftHipIndex] - jointsPos[leftKneeIndex];
            Vector3 rvKneeToHip = jointsPos[rightHipIndex] - jointsPos[rightKneeIndex];
            Vector3 lvKneeToAnkle = jointsPos[leftAnkleIndex] - jointsPos[leftKneeIndex];
            Vector3 rvKneeToAnkle = jointsPos[rightAnkleIndex] - jointsPos[rightAnkleIndex];

            //float lkAngle = Vector3.Angle(lvKneeToHip, lvKneeToAnkle);
            //float rkAngle = Vector3.Angle(rvKneeToHip, rvKneeToAnkle);

            Vector3 vectorA = jointsPos[neckIndex] - jointsPos[leftHandIndex];
            Vector3 vectorB = jointsPos[neckIndex] - jointsPos[rightHandIndex];
            float angleCompare = Vector2.Angle(new Vector2(vectorA.x, vectorA.y), new Vector2(vectorB.x, vectorB.y));
            ...
            switch (gestureData.gesture)
            {
                    ...
                    switch (gestureData.state)
                    {
                        case 0:  // gesture detection
                            if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] &&
                                ...
                                (jointsPos[rightElbowIndex].x - jointsPos[rightShoulderIndex].x) > 0.05f &&
                                ...
                                (jointsPos[leftElbowIndex].x - jointsPos[leftShoulderIndex].x) < 0.05f)
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                            }
                            break;
                ...
                case GestureType.Qg_PrePose://掌抱腹前-预备式
                    switch (gestureData.state)
                    {
                        case 0: //moveTracked
                            if (jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftHandIndex] && 
                                jointsTracked[rightHandIndex] && jointsTracked[leftFingerIndex] && jointsTracked[rightFingerIndex] &&
                                (jointsPos[leftShoulderIndex].y - jointsPos[leftHandIndex].y>=0.1f) && (jointsPos[rightShoulderIndex].y- jointsPos[rightHandIndex].y >=0.1f) &&
                                (jointsPos[leftHandIndex].z - jointsPos[leftShoulderIndex].z <= -0.15f) && (jointsPos[rightHandIndex].z - jointsPos[rightHipIndex].z <= -0.15f) &&
                                (Vector3.Distance(jointsPos[leftFingerIndex], jointsPos[rightFingerIndex]) <= 0.2f)
                                )//手低于肩膀，向前环抱，手指相近
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                            ...
                            break;
                        case 1:
                            bool isInPose = jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftHandIndex] &&
                                jointsTracked[rightHandIndex] && jointsTracked[leftFingerIndex] && jointsTracked[rightFingerIndex] &&
                                (jointsPos[leftShoulderIndex].y - jointsPos[leftHandIndex].y >= 0.1f) && (jointsPos[rightShoulderIndex].y - jointsPos[rightHandIndex].y >= 0.1f) &&
                                (jointsPos[leftHandIndex].z - jointsPos[leftShoulderIndex].z <= -0.05f) && (jointsPos[rightHandIndex].z - jointsPos[rightHipIndex].z <= -0.05f) &&
                                (Vector3.Distance(jointsPos[leftFingerIndex], jointsPos[rightFingerIndex]) <= 0.2f);
                ...
                case GestureType.QgMove1_1://两手托天理三焦
                    switch (gestureData.state)
                    {
                        case 0:  // gesture detection - phase 1
                            if (jointsTracked[leftFingerIndex] && jointsTracked[rightFingerIndex] && jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] 
                                && jointsTracked[rightElbowIndex] && jointsTracked[leftElbowIndex] &&
                                jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] && jointsTracked[leftKneeIndex] &&
                                jointsTracked[rightKneeIndex] && jointsTracked[leftAnkleIndex] && jointsTracked[rightAnkleIndex] &&
                                (jointsPos[rightAnkleIndex].x > jointsPos[rightKneeIndex].x) &&
                                (jointsPos[leftAnkleIndex].x < jointsPos[leftKneeIndex].x)) //双脚站开
                        ...
                        case 1:  // gesture phase 2 = complete
                            if ((timestamp - gestureData.timestamp) < 3.5f)//3.5秒上升期
                            {
                                bool isInPose = jointsTracked[leftFingerIndex] && jointsTracked[rightFingerIndex] && jointsTracked[leftShoulderIndex] 
                                && jointsTracked[rightShoulderIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftElbowIndex] && 
                                (jointsPos[leftElbowIndex].y > jointsPos[leftShoulderIndex].y) && (jointsPos[leftFingerIndex].y > jointsPos[leftElbowIndex].y) && 
                                (jointsPos[rightElbowIndex].y > jointsPos[rightShoulderIndex].y) && (jointsPos[rightFingerIndex].y > jointsPos[rightElbowIndex].y)
                                 && Vector3.Distance(jointsPos[leftFingerIndex], jointsPos[rightFingerIndex]) <= 0.12f && jointsPos[leftFingerIndex].y - jointsPos[noseIndex].y >= 0.1f &&
                                 (jointsPos[rightAnkleIndex].x > jointsPos[rightKneeIndex].x) && (jointsPos[leftAnkleIndex].x < jointsPos[leftKneeIndex].x)
                                 ; //手合并，举过头顶
                                {
                                    Vector3 jointPos = jointsPos[gestureData.joint];
                                    CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f);
                ...
                case GestureType.QgMove2_1://左右开弓似射雕搭腕
                    switch (gestureData.state)
                    {
                        case 0: //moveTracked
                            if (jointsTracked[leftHandIndex] &&jointsTracked[rightHandIndex] &&
                                Vector3.Distance(jointsPos[leftHandIndex], jointsPos[rightHandIndex]) <= 0.12f 
                                && jointsPos[leftHandIndex].y >= gestureBottom && jointsPos[leftHandIndex].y <= gestureTop &&
                                jointsPos[rightHandIndex].y >= gestureBottom && jointsPos[rightHandIndex].y <= gestureTop)//双手在肢干区域
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                            }
                        ...
                        case 1:
                            if ((timestamp - gestureData.timestamp) < 1.5f)
                            {
                                ...
                                Vector3.Distance(jointsPos[leftHandIndex], jointsPos[rightHandIndex]) <= 0.12f);//双手靠近，完成搭腕
                ...
                case GestureType.QgMove2_2L://左右开弓似射雕，左开弓
                    switch (gestureData.state)
                    {
                        case 0: //moveTracked
                            if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[leftElbowIndex] &&
                                jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftShoulderIndex] &&
                                Vector3.Distance(jointsPos[rightHandIndex], jointsPos[rightShoulderIndex]) <= 0.12f && (jointsPos[leftHandIndex].x - jointsPos[leftElbowIndex].x < -0.05f)
                                && (jointsPos[leftElbowIndex].x - jointsPos[leftShoulderIndex].x < -0.05f) && Mathf.Abs(jointsPos[leftHandIndex].y - jointsPos[leftElbowIndex].y) < 0.1f &&
                                Mathf.Abs(jointsPos[leftElbowIndex].y - jointsPos[leftShoulderIndex].y) < 0.1f)//左手平直，右手靠近肩膀做拉弓动作
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                            }
                        ...
                        case 1:
                            if ((timestamp - gestureData.timestamp) < 1.5f)
                            {
                                bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftShoulderIndex] &&
                                   Vector3.Distance(jointsPos[rightHandIndex], jointsPos[rightShoulderIndex]) <= 0.12f && (jointsPos[leftHandIndex].x- jointsPos[leftElbowIndex].x < -0.05f)
                                && (jointsPos[leftElbowIndex].x - jointsPos[leftShoulderIndex].x < -0.05f);
                ...
                case GestureType.QgMove2_2R://左右开弓似射雕，右开弓
                    switch (gestureData.state)
                    {
                        case 0: //moveTracked
                            if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[leftElbowIndex] &&
                                jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftShoulderIndex] &&
                                Vector3.Distance(jointsPos[leftHandIndex], jointsPos[leftShoulderIndex]) <= 0.12f && (jointsPos[rightHandIndex].x - jointsPos[rightElbowIndex].x > 0.05f)
                                && (jointsPos[rightElbowIndex].x - jointsPos[rightShoulderIndex].x > 0.05f) && Mathf.Abs(jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) < 0.1f &&
                                Mathf.Abs(jointsPos[rightElbowIndex].y - jointsPos[rightShoulderIndex].y) < 0.1f)//右手平直，左手靠近左肩膀做拉弓动作
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                            }
                        ...
                        case 1:
                            if ((timestamp - gestureData.timestamp) < 1.5f)
                            {
                                bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightShoulderIndex] &&
                                   Vector3.Distance(jointsPos[leftHandIndex], jointsPos[leftShoulderIndex]) <= 0.12f && (jointsPos[rightHandIndex].x - jointsPos[rightElbowIndex].x > 0.05f)
                                && (jointsPos[rightElbowIndex].x - jointsPos[rightShoulderIndex].x > 0.05f);
                ...
                case GestureType.QgMove3_2L://调理脾胃需单举，举左手
                    switch (gestureData.state)
                    {
                        case 0: //moveTracked
                            if (jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftHandIndex] &&
                                jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftElbowIndex] &&
                                (jointsPos[leftHandIndex].y > jointsPos[leftElbowIndex].y) && (jointsPos[leftElbowIndex].y > jointsPos[leftShoulderIndex].y) &&
                                (jointsPos[rightHandIndex].y < jointsPos[rightElbowIndex].y) && (jointsPos[rightElbowIndex].y < jointsPos[leftShoulderIndex].y))
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                            }
                        ...
                        case 1:
                            bool isInPose = jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftHandIndex] &&
                                jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftElbowIndex] &&
                                (jointsPos[leftHandIndex].y > jointsPos[leftElbowIndex].y) && (jointsPos[leftElbowIndex].y > jointsPos[leftShoulderIndex].y) &&
                                (jointsPos[rightHandIndex].y < jointsPos[rightElbowIndex].y) && (jointsPos[rightElbowIndex].y < jointsPos[leftShoulderIndex].y);//左手高于肘，肘高于肩膀
                ...
                case GestureType.QgMove3_2R://调理脾胃需单举，举右手
                    switch (gestureData.state)
                    {
                        case 0: //moveTracked
                            if (jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftHandIndex] &&
                                jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftElbowIndex] &&
                                (jointsPos[rightHandIndex].y > jointsPos[rightElbowIndex].y) && (jointsPos[rightElbowIndex].y > jointsPos[rightShoulderIndex].y) &&
                                (jointsPos[leftHandIndex].y < jointsPos[leftElbowIndex].y) && (jointsPos[leftElbowIndex].y < jointsPos[leftShoulderIndex].y))
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                ...
                case GestureType.QgMove4_2://五劳七伤往后瞧
                    switch (gestureData.state)
                    {
                        case 0:  // gesture detection - phase 1
                            if (jointsTracked[leftElbowIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftHandIndex] &&
                                jointsTracked[rightHandIndex] && jointsTracked[neckIndex] && 
                                 (angleCompare >= 120f && angleCompare <= 160f) &&
                               jointsPos[leftHandIndex].x - jointsPos[leftElbowIndex].x < 0.07f && jointsPos[rightHandIndex].x - jointsPos[rightElbowIndex].x > 0.07f)//双手展开，与脖子成一定夹角
                            {
                                SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
                                gestureData.progress = 0.5f;
                            }
                        ...
                        case 1:  // gesture phase 2 = complete
                            if ((timestamp - gestureData.timestamp) < 3.5f)
                            {
                                bool isInPose = jointsTracked[leftElbowIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftHandIndex] &&
                                jointsTracked[rightHandIndex] && jointsTracked[neckIndex] && jointsTracked[noseIndex] &&
                                 Mathf.Abs(jointsPos[neckIndex].x - jointsPos[noseIndex].x) > 0.03f &&
                                (angleCompare >= 120f && angleCompare <= 160f) &&
                                jointsPos[leftHandIndex].x - jointsPos[leftElbowIndex].x < -0.07f && jointsPos[rightHandIndex].x - jointsPos[rightElbowIndex].x > 0.07f;//扭动脖子向后看
                ...
                case GestureType.QgMove5_2://摇头摆尾去心火，左倾/左旋
                    switch (gestureData.state)
                    {
                        case 0:  
                            if (jointsTracked[rightHipIndex] && jointsTracked[neckIndex] &&
                                jointsTracked[rightShoulderIndex] && jointsTracked[leftShoulderIndex] && jointsTracked[rightHandIndex] && jointsTracked[leftHandIndex] &&
                               (jointsPos[rightShoulderIndex].x - jointsPos[rightHipIndex].x) < 0f && (jointsPos[rightShoulderIndex].y- jointsPos[rightHandIndex].y > 0.1f) 
                               && (jointsPos[rightHipIndex].y - jointsPos[rightHandIndex].y > 0.03f))//右手要低于胯
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightShoulderIndex, jointsPos[rightShoulderIndex]);
                                gestureData.progress = 0.3f;
                            }
                        ...
                        case 1:  // gesture phase 2 = complete
                            if ((timestamp - gestureData.timestamp) < 0.5f)
                            {
                                bool isInPose = jointsTracked[rightShoulderIndex] && jointsTracked[rightHipIndex] && jointsTracked[neckIndex] &&
                                    jointsTracked[rightShoulderIndex] && jointsTracked[leftShoulderIndex] && jointsTracked[rightHandIndex] && jointsTracked[leftHandIndex] &&
                                    (jointsPos[rightShoulderIndex].x - jointsPos[rightHipIndex].x) < 0f;//右肩膀向左倾斜，左倾角度大于右胯

                                if (isInPose)
                                {
                                    Vector3 QgLean1 = jointsPos[neckIndex] - jointsPos[hipCenterIndex];
                                    gestureData.screenPos.z = Vector3.Angle(Vector3.up, QgLean1);//计算倾角
                                    Vector3 jointPos = jointsPos[gestureData.joint];
                                    CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f);
                                    gestureData.timestamp = timestamp;
                                    gestureData.progress = 0.7f;
                ...
                case GestureType.QgMove5_3://摇头摆尾去心火，右倾/右旋
                    switch (gestureData.state)
                    {
                        case 0:  
                            if (jointsTracked[leftHipIndex] && jointsTracked[neckIndex] &&
                                jointsTracked[rightShoulderIndex] && jointsTracked[leftShoulderIndex] && jointsTracked[rightHandIndex] && jointsTracked[leftHandIndex] &&
                               (jointsPos[leftShoulderIndex].x - jointsPos[leftHipIndex].x) > 0f &&  (jointsPos[leftShoulderIndex].y- jointsPos[leftHandIndex].y > 0.1f)
                               && (jointsPos[leftHipIndex].y - jointsPos[leftHandIndex].y > 0.03f))//右手要低于胯
                            {
                                SetGestureJoint(ref gestureData, timestamp, leftShoulderIndex, jointsPos[leftShoulderIndex]);
                                gestureData.progress = 0.3f;
                            }
                        ...
                        case 1:  // gesture phase 2 = complete
                            if ((timestamp - gestureData.timestamp) < 0.5f)
                            {
                                bool isInPose = (jointsTracked[leftHipIndex] && jointsTracked[neckIndex] &&
                                    jointsTracked[rightShoulderIndex] && jointsTracked[leftShoulderIndex] && jointsTracked[rightHandIndex] && jointsTracked[leftHandIndex]) &&
                                    (jointsPos[leftShoulderIndex].x - jointsPos[leftHipIndex].x) > 0f;//右肩膀向左倾斜，左倾角度大于右胯

                                if (isInPose)
                                {
                                    Vector3 QgLean2 = jointsPos[neckIndex] - jointsPos[hipCenterIndex];
                                    gestureData.screenPos.z = Vector3.Angle(Vector3.up, QgLean2);//计算倾角
                                    Vector3 jointPos = jointsPos[gestureData.joint];
                                    CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f);
                                    gestureData.timestamp = timestamp;
                                    gestureData.progress = 0.7f;
                ...
                case GestureType.QgMove5_4://摇头摆尾去心火，左旋或右旋时身体向前倾
                    switch (gestureData.state)
                    {
                        case 0:  
                            if (jointsTracked[neckIndex] && jointsTracked[hipCenterIndex] &&
                                (jointsPos[neckIndex].z - jointsPos[hipCenterIndex].z) < -0.1f)//脖子比胯胯前
                            {
                                SetGestureJoint(ref gestureData, timestamp, neckIndex, jointsPos[neckIndex]);
                                gestureData.progress = 0.3f;
                            }
                        ...
                        case 1:  // gesture phase 2 = complete
                            ...
                            {
                                bool isInPose = jointsTracked[neckIndex] && jointsTracked[hipCenterIndex] &&
                                    (jointsPos[neckIndex].z - jointsPos[leftHipIndex].z) < -0.1f;//继续判定到脖子比胯靠前

                                if (isInPose)
                                {
                                    Vector3 QgLean3 = jointsPos[neckIndex] - jointsPos[hipCenterIndex];
                                    gestureData.screenPos.z = Vector3.Angle(Vector3.up, QgLean3);//计算倾角
                                    Vector3 jointPos = jointsPos[gestureData.joint];
                                    CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f);
                                    gestureData.timestamp = timestamp;
                                    gestureData.progress = 0.7f;
                ...
                case GestureType.QgMove6_1://两手攀足固肾腰
                    switch (gestureData.state)
                    {
                        case 0: //moveTracked
                            if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[neckIndex] && jointsTracked[hipCenterIndex] &&
                                jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftElbowIndex] &&
                                jointsTracked[leftKneeIndex] && jointsTracked[rightKneeIndex]
                                && jointsPos[leftElbowIndex].y > jointsPos[leftHandIndex].y  && jointsPos[leftShoulderIndex].y > jointsPos[leftElbowIndex].y &&
                                jointsPos[rightElbowIndex].y > jointsPos[rightHandIndex].y && jointsPos[rightShoulderIndex].y > jointsPos[rightElbowIndex].y &&
                                (jointsPos[neckIndex].z - jointsPos[hipCenterIndex].z) < -0.1f)//身体向前弯曲，
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                            }
                        ...
                        case 1:
                            if((timestamp - gestureData.timestamp) < 1.5f)
                            {
                               bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[neckIndex] && jointsTracked[hipCenterIndex] &&
                              jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[rightElbowIndex] && jointsTracked[leftElbowIndex] &&
                              jointsTracked[leftKneeIndex] && jointsTracked[rightKneeIndex] &&
                              Vector3.Distance(jointsPos[leftHandIndex], jointsPos[leftKneeIndex]) <= 0.12f && Vector3.Distance(jointsPos[rightHandIndex], jointsPos[rightKneeIndex]) <= 0.12f
                              && jointsPos[leftElbowIndex].y > jointsPos[leftHandIndex].y && jointsPos[leftShoulderIndex].y > jointsPos[leftElbowIndex].y &&
                              jointsPos[rightElbowIndex].y > jointsPos[rightHandIndex].y && jointsPos[rightShoulderIndex].y > jointsPos[rightElbowIndex].y &&
                              (jointsPos[neckIndex].z - jointsPos[hipCenterIndex].z) < -0.1f;//手碰到膝盖
                ...
                case GestureType.QgMove7_1://攒拳怒目增气力，抱拳
                    switch (gestureData.state)
                    {
                        case 0: //moveTracked
                            if (jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[leftHandIndex] &&
                                jointsTracked[rightHandIndex] && jointsTracked[rightHipIndex] && jointsTracked[leftHipIndex] &&
                                (jointsPos[leftHandIndex].y < jointsPos[leftShoulderIndex].y) && (jointsPos[rightHandIndex].y < jointsPos[rightShoulderIndex].y) &&
                                (Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[leftHipIndex].x)) <= 0.1f && (Mathf.Abs(jointsPos[leftHandIndex].y - jointsPos[leftHipIndex].y)) <= 0.1f &&
                                (Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightHandIndex].x)) <= 0.1f && (Mathf.Abs(jointsPos[rightHandIndex].y - jointsPos[rightHandIndex].y)) <= 0.1f
                                )
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                                gestureData.progress = 0.5f;
                ...
                case GestureType.QgMove7_2L://攒拳怒目增气力，出左拳
                    switch (gestureData.state)
                    {
                        case 0:  // gesture detection - phase 1
                ...
                case GestureType.QgMove7_2R://攒拳怒目增气力，出左拳
                    switch (gestureData.state)
                    {
                        case 0:  // gesture detection - phase 1
                            if (jointsTracked[rightHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightShoulderIndex] &&
                ...
                case GestureType.QgMove8_1://背后七颠百病消，踮脚
                    float initialDisR = Mathf.Abs(jointsPos[rightAnkleIndex].y - jointsPos[rightFootIndex].y);
                    float initialDisL = Mathf.Abs(jointsPos[leftAnkleIndex].y - jointsPos[leftFootIndex].y);
                    switch (gestureData.state)
                    {
                        case 0:  // gesture detection - phase 1
                            if (jointsTracked[leftFingerIndex] && jointsTracked[rightFingerIndex] &&
                                jointsTracked[leftFootIndex] && jointsTracked[rightFootIndex] &&
                                jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] && jointsPos[leftFingerIndex].y - jointsPos[leftHipIndex].y <0.001f && jointsPos[rightFingerIndex].y - jointsPos[rightHipIndex].y < 0.001f
                                && Mathf.Abs(jointsPos[leftFingerIndex].x - jointsPos[leftHipIndex].x) <= 0.1f && Mathf.Abs(jointsPos[rightFingerIndex].x - jointsPos[rightHipIndex].x) <= 0.1f
                                && Mathf.Abs(jointsPos[rightAnkleIndex].x - jointsPos[rightFootIndex].x) <= 0.05f && Mathf.Abs(jointsPos[leftAnkleIndex].x - jointsPos[leftFootIndex].x) <= 0.05f
                                && initialDisR > 0.01f && initialDisL > 0.01f
                                )
                            {
                                SetGestureJoint(ref gestureData, timestamp, rightHipIndex, jointsPos[rightHipIndex]);
                                gestureData.initialFootL = initialDisL;
                                gestureData.initialFootR = initialDisR;
                                gestureData.progress = 0.3f;
                            }
                            break;
                        ...
                        case 1:  // gesture phase 2 = complete
                            if ((timestamp - gestureData.timestamp) < 1.5f)
                            {
                                float finalDisR = Mathf.Abs(jointsPos[rightAnkleIndex].y - jointsPos[rightFootIndex].y);
                                float finalDisL = Mathf.Abs(jointsPos[leftAnkleIndex].y - jointsPos[leftFootIndex].y);

                                bool isInPose = jointsTracked[rightAnkleIndex] && jointsTracked[leftFootIndex] &&
                                jointsTracked[rightFootIndex] && jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] &&
                                jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] && jointsTracked[leftAnkleIndex] &&
                                jointsPos[leftFingerIndex].y - jointsPos[leftHipIndex].y < 0.001f && jointsPos[rightFingerIndex].y - jointsPos[rightHipIndex].y < 0.001f
                                && Mathf.Abs(jointsPos[leftFingerIndex].x - jointsPos[leftHipIndex].x) <= 0.1f && Mathf.Abs(jointsPos[rightFingerIndex].x - jointsPos[rightHipIndex].x) <= 0.1f &&
                                Mathf.Abs(finalDisR) - Mathf.Abs(gestureData.initialFootR) > 0.01f && Mathf.Abs(finalDisL) - Mathf.Abs(gestureData.initialFootL) > 0.01f && initialDisR > 0.01f && initialDisL > 0.01f;
